...
class myJAAD(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return obs_p, obs_s, self.frames_in[idx], obs_m, self.frames_out[idx], velocity_obs_m
...
def my_collate(batch):
    ...
    (obs_p, obs_s, obs_f, obs_m, target_f, vo_m) = zip(*batch)
    ...
    vo_m = torch.cat(vo_m, dim=0).permute(1,0,2)
    seq_start_end = torch.LongTensor(seq_start_end)
    out = [obs_p, obs_s, obs_ff, obs_m, target_ff, seq_start_end, vo_m]
    return tuple(out)
