...
def myVIM(pred, true, mask):
    ...
    if(scale):
        true = true.view(seq_len, batch, l//2, 2)
        pred = pred.view(seq_len, batch, l//2, 2)
        true[:,:,:,0] *= scalex
        true[:,:,:,1] *= scaley
        pred[:,:,:,0] *= scalex
        pred[:,:,:,1] *= scaley
        true = true.view(seq_len, batch, l)
        pred = pred.view(seq_len, batch, l)
...
def ADE_c(pred, true):
    ...
    displacement=torch.sqrt(torch.sum((pred-true)**2, dim=-1) / 14. )
...
def FDE_c(pred, true):
    ...
    displacement=torch.sqrt(torch.sum((pred[-1,:,:]-true[-1,:,:])**2, -1) / 14. )
...
def speed2pos(preds, obs_p):
    ...
    pred_pos = torch.zeros(seq_len, batch, l).to('cuda')
    current = obs_p[-1,:,:]
    ...
    #if(scale):
    #    pred_pos = torch.min(pred_pos, torch.ones(seq_len, batch, l, device='cuda'))
    #    pred_pos = torch.max(pred_pos, torch.zeros(seq_len, batch, l, device='cuda'))
    #else:
    #    pred_pos = torch.min(pred_pos, 1280*torch.ones(seq_len, batch, l, device='cuda'))
    #    pred_pos = torch.max(pred_pos, torch.zeros(seq_len, batch, l, device='cuda'))
