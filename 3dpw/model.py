...
class PoolHiddenNet(nn.Module):
    ...
    def __init__(
        self, embedding_dim=39, h_dim=64, mlp_dim=1024, bottleneck_dim=1024,
...
class LSTM(nn.Module):
    def __init__(self, embedding_dim=39, h_dim=256, num_layers=1, bottleneck_dim = 1024, mlp_dim = 1024, dropout=0.0):
        ...
        self.hidden2pose = nn.Linear(h_dim, embedding_dim)
        #self.hidden2pose = nn.Sequential(nn.Linear(h_dim, embedding_dim), nn.Hardtanh(min_val=-MAX_VAL, max_val=MAX_VAL))
        #self.hidden2pose = nn.Sequential(nn.Linear(h_dim, embedding_dim), nn.LeakyReLU(0.1))
    ...
    def forward(self, obs_pose=None, obs_speed=None, start_end_idx=None, pred_len=14):
        ...
        for _ in range(pred_len):
             ...
             #curr_speed = last_speed.squeeze(0) + self.hidden2pose(output.view(-1, self.h_dim))
             pred_speed = torch.cat((pred_speed, curr_speed.unsqueeze(0)), dim=0)
             curr_pose = curr_pose + curr_speed + 0.5*(curr_speed - last_speed.squeeze(0))
             pred_pose = torch.cat((pred_pose, curr_pose.unsqueeze(0)), dim=0)
             last_speed = curr_speed.unsqueeze(0)
 
        return pred_speed, pred_pose
