...
import model_vae 
net_g = model_vae.LSTM_local_global().to(args.device).double()
encoder = model_vae.Encoder(pose_dim=39, h_dim=32, latent_dim=16)
decoder = model_vae.Decoder(pose_dim=39, h_dim=32, latent_dim=16)
net_l = model_vae.VAE(Encoder=encoder, Decoder=decoder)
if torch.cuda.is_available():
    net_l.cuda()
net_l.double()

optimizer = optim.Adam( list(net_l.parameters()) + list(net_g.parameters()), lr=0.004)
...
for epoch in range(args.n_epochs):
    ...
    for idx, (obs_p, obs_s, obs_f, target_p, target_s, target_f, start_end_idx) in enumerate(train):
        ...
        elif(MODEL_VERSION == 'neck'):
            obs_s_g = obs_s.view(15, batch, 13, 3)[:,:,6]
            target_s_g = target_s.view(14, batch, 13, 3)[:,:,6]
        elif(MODEL_VERSION == 'rankle'):
            obs_s_g = obs_s.view(15, batch, 13, 3)[:,:,4]
            target_s_g = target_s.view(14, batch, 13, 3)[:,:,4]
        else:
            print("ERROR")
            exit()
        ###########################################################
        ...
        loss_g = mse(speed_preds_g, target_s_g)
        ###########################################################
        ...
        output, mean, log_var = net_l(obs_s_l.double())
        target_s_l = (target_s.view(14, batch, 13, 3) - target_s_g.view(14, batch, 1, 3)).view(14, batch, 39)
        loss_l = model_vae.vae_loss_function(target_s_l.double(), output, mean, log_var) #- 0.0001* torch.norm(output)
        ########################################################################
        ...
        #########################debug#############################
        if(epoch%10 == 0): print(f"deb: loss_g {loss_g.item()} loss_l {coeff*loss_l.item()}")
    ...
    for idx, (obs_p, obs_s, obs_f, target_p, target_s, target_f, start_end_idx) in enumerate(val_all):
        ...
        with torch.no_grad():
            ...
            output, mean, log_var = net_l(obs_s_l.double())
            target_s_l = (target_s.view(14, batch, 13, 3) - target_s_g.view(14, batch, 1, 3)).view(14, batch, 39)
            loss_l = model_vae.vae_loss_function(target_s_l.double(), output, mean, log_var)
            ########################################################################
    ...
    if(vim_val < best_model_val):
        ...
        for idx, (obs_p, obs_s, obs_f, target_f, start_end_idx) in enumerate(test):
            ...
            with torch.no_grad():
                obs_s_l = (obs_s.view(15, batch, 13, 3) - obs_s_g.view(15, batch, 1, 3)).view(15, batch, 39)
                pred_s_g = net_g(global_s=obs_s_g)
                output, mean, log_var = net_l(obs_s_l.double())
                speed_preds = (pred_s_g.view(14, batch, 1, 3) + output.view(14, batch, 13, 3)).view(14, batch, 39)
                preds_p = utils.speed2pos(speed_preds, obs_p) 
        ...
        if(vim_val < 0.155):
            visu("disentangledVAE", obs_p_val.permute(1,0,2).view(-1, 2, 16, 13, 3), preds_p_val.permute(1,0,2).view(-1, 2, 14, 13, 3), target_p_val.permute(1,0,2).view(-1, 2, 14, 13, 3), obs_f_val, target_f_val)
        ###############################################################################
