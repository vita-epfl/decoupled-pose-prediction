...
class LSTM_g(nn.Module):
    def __init__(self, D_dim=2, embedding_dim=8, h_dim=16, num_layers=1, dropout=0.2, pred_len=16):
        ...
        self.embedding_fn = nn.Sequential(nn.Linear(D_dim, embedding_dim), nn.Tanh())
        ...
        self.hidden2g = nn.Sequential(nn.Linear(h_dim, D_dim))

    def forward(self, global_s=None):

        seq_len, batch, l = global_s.shape
        state_tuple_g = (torch.zeros(self.num_layers, batch, self.h_dim, device='cuda' if torch.cuda.is_available() else 'cpu', dtype=torch.float64),
                       torch.zeros(self.num_layers, batch, self.h_dim, device='cuda' if torch.cuda.is_available() else 'cpu', dtype=torch.float64))

        global_s = global_s.contiguous()
        output_g, state_tuple_g = self.encoder_g(self.embedding_fn(global_s.view(-1,self.D_dim)).view(seq_len, batch, self.embedding_dim), state_tuple_g)

        pred_s_g = torch.tensor([], device='cuda' if torch.cuda.is_available() else 'cpu', dtype=torch.float64)
        last_s_g = global_s[-1].unsqueeze(0) 
        for _ in range(self.pred_len):

             output_g, state_tuple_g = self.decoder_g(self.embedding_fn(last_s_g.view(-1,self.D_dim)).view(1, batch, self.embedding_dim), state_tuple_g)
...
class Encoder(nn.Module):
    ...
    def forward(self, obs_s=None):
        ...
        batch = obs_s.size(1)
        state_tuple = (torch.zeros(self.num_layers, batch, self.h_dim, device='cuda' if torch.cuda.is_available() else 'cpu', dtype=torch.float64),
                       torch.zeros(self.num_layers, batch, self.h_dim, device='cuda' if torch.cuda.is_available() else 'cpu', dtype=torch.float64))
...
class Decoder(nn.Module):
    def __init__(self, pose_dim=34, h_dim=32, latent_dim=16, num_layers=1, dropout=0.3, pred_len=16):
    ...
    def forward(self, obs_s=None, latent=None):

        batch = obs_s.size(1)
        decoder_c = torch.zeros(self.num_layers, batch, self.h_dim, device='cuda' if torch.cuda.is_available() else 'cpu', dtype=torch.float64)
        last_s = obs_s[-1].unsqueeze(0)
        decoder_h = self.FC(latent).unsqueeze(0)
        decoedr_h = decoder_h.repeat(self.num_layers, 1, 1)
        state_tuple = (decoder_h, decoder_c)

        preds_s = torch.tensor([], device='cuda' if torch.cuda.is_available() else 'cpu', dtype=torch.float64)
        for _ in range(self.pred_len):
